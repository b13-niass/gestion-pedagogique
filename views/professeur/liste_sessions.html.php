...
<div data-iduser="<?=$user->id?>" data-idcour="<?=$cour->id?>" class="bg-box-dark rounded-10 sm:p-[40px] p-[20px]">
...
<script>
    document.addEventListener('DOMContentLoaded', async function() {
        ...
        const cour_id = document.querySelector('[data-idcour]').dataset.idcour;
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: async function(info, successCallback, failureCallback) {
                try {
                    const response = await fetch(`/api/prof/${user_id}/cours/${cour_id}/sessions`);
                    if (!response.ok) {
